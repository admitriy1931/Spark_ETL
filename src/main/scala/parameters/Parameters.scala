...
class Parameters(args: Array[String])(implicit log: Logger) {
  ...
  private def getValue(key: String): String = {
    this.argsMap.getOrElse(key, println("Ex").toString)
  }
  private val argsMap: Map[String, String] = args.map(pair).toMap

  val W4_SHO_CLIENT_TABLE: String = getValue("W4_SOH_CLIENT_TABLE")
  val W4_SHO_CONTRACT_TABLE: String = getValue("W4_SOH_ACNT_CONTRACT_TABLE")
